...
export const PaymentFormSection = ({
  ...
  const orderItems = useMemo(() => {
    ...
    return Array.from({ length: safeCount }, (_, idx) => ({
      ...
      name: "Name xxx",
      qty: 1,
      price: generatePrice("checkout", `Sản phẩm #${idx + 1}`),
      image: productPh,
    }));
  }, [cartCount]);
      ...
      <div className="w-[420px] bg-gray-00 rounded border border-solid border-gray-100 p-6 shrink-0">
        ...
        <div className="flex flex-col gap-3 mt-4">
          <div className="flex justify-between">
            ...
            <span>{formatVND(subtotalNumber)}</span>
          </div>
          <div className="flex justify-between"><span className="text-gray-600">Phí vận chuyển</span><span>Free</span></div>
          <div className="flex justify-between"><span className="text-gray-600">Giảm giá</span><span>{formatVND(0)}</span></div>
          <div className="flex justify-between pt-3 border-t border-solid border-gray-100">
            ...
            <span className="font-body-medium-600 text-gray-900">{formatVND(subtotalNumber)}</span>
          </div>
        </div>
        <button
          type="button"
