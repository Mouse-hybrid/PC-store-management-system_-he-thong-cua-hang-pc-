...
export const ShoppingCartSection = ({ cartCount = 0, onGoShop, onGoCheckout }) => {
  ...
  const pricePresets = [70, 80, 220, 250, 360, 1200, 1500, 2300];
  ...
  const items = useMemo(() => {
    ...
    return Array.from({ length: cartCount }, (_, idx) => ({
      ...
      name: "Name xxx",
      image: productPh,
      price: pricePresets[idx % pricePresets.length],
      qty: 1,
    }));
  }, [cartCount]);
  ...
  const subtotal = formatUsd(subtotalValue);
  const shipping = "Free";
  const discount = "$0";
  const total = formatUsd(subtotalValue);
                  ...
                  {pagedItems.map((it) => (
                    ...
                    >
                      <div className="col-span-2 inline-flex items-center gap-3">
                        <img className="w-12 h-12 object-cover" alt="" src={it.image} />
                      </div>
                      <div className="col-span-2 text-gray-700 text-sm">{formatUsd(it.price)}</div>
